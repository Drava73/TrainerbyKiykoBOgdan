...
BOOL Trainer::Cls_OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
    ...
    SetClassLong(hwnd,  -14, LONG(hIcon));
...
void Trainer::Cls_OnCommand(HWND hwnd, int id, HWND hwndCtrl, UINT codeNotify)
{
    if (id == IDOK) {
        TCHAR path[50];
        if (BST_CHECKED == SendMessage(hRadioLight, BM_GETCHECK, NULL, NULL)) {                              
...
LRESULT CALLBACK Trainer::DlgProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_KEYDOWN: {
        if ((LOWORD(wParam) >= 65 && LOWORD(wParam) <= 90) || (LOWORD(wParam) >= 97 && LOWORD(wParam) <= 122) || LOWORD(wParam) == 32) {//ïðîâåðêà êîäà íàæàòîé êëàâèøè(äèàïàçîí ëàòèíèöû è ïðîáåë)
